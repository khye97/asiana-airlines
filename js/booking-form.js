...
$(".fsw").load("/include/booking-form.html", function () {
  ...
  $("#upgrade-only").on("change", function () {
    if ($(this).is(":checked")) {
      ...
      $economyStandardBtn.prop("disabled", false);
    } else {
      $(".fsw-modal__seat-grade-btn-option").prop("disabled", false);
  ...
  function clickDifferentBtn($target) {
    ...
    seatAddClass($target);
    $otherAllBtns.prop("disabled", false);
  ...
  $("#separate-round-trip").on("change", function () {
    ...
    if ($(this).is(":checked")) {
      ...
      // departure의 is-selected 버튼과 return의 is-selected 버튼이 같은 경우
